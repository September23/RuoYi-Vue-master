<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="状态(0上班 1下班 2请假)" prop="clockStatus">
        <el-input
          v-model="queryParams.clockStatus"
          placeholder="请输入状态(0上班 1下班 2请假)"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="clockList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="打卡时间" align="center" prop="clockTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.clockTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态(0上班 1下班 2请假)" align="center" prop="clockStatus" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="员工名字" prop="userName">
          <el-input v-model="form.userName" placeholder="请输入员工名字" />
        </el-form-item>
        <el-form-item label="部门编号" prop="deptId">
        ...
        <el-form-item label="状态(0上班 1下班 2请假)" prop="clockStatus">
          <el-input v-model="form.clockStatus" placeholder="请输入状态(0上班 1下班 2请假)" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除打卡信息编号为"' + userIds + '"的数据项？').then(function() {
        return delClock(userIds);
      }).then(() => {
        this.getList();
